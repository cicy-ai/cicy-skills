---
name: claude-auth
description: Back up and restore this host's Claude credential (~/.claude/.credentials.json) as a base64 file. Export writes to a 0600 file, import validates the decoded JSON and keeps a timestamped backup.
---

# Claude Auth

Move this host's Claude credential between the live file and a base64 text
blob — for backing it up, or carrying it to another machine.

Credential file: `~/.claude/.credentials.json` (override with `CLAUDE_AUTH_PATH=<absolute path>`).

## Scope

Use this skill when the task is to:

- back up the Claude credential before reinstalling or re-authenticating,
- copy an existing Claude login onto another host,
...
```
claude-auth export [--out <file>]      base64 → file, mode 0600
claude-auth export --stdout            base64 → stdout (only when you mean it)
claude-auth import --file <file>       restore from a base64 file
claude-auth import --base64 <value>    restore from an inline value
claude-auth import -                   restore from stdin
claude-auth status [--json]            path, size, mode, mtime — never contents
claude-auth path                       print the credential path
```

With no `--out`, `export` writes `~/cicy-ai/assets/claude-auth-<timestamp>.b64`.
...
5. **Already-running Claude processes keep their old credential.** Only
