...
description: Install the latest cicy-mobile build onto a phone plugged into this Mac by USB — runs locally (no ssh). Android via adb install; iOS guided two ways: Sideloadly IPA re-sign or Xcode source build.
---

# Cicy Mobile Install

Install the latest **cicy-mobile** build onto a phone that's plugged into **this
Mac** by USB. The skill runs **locally** — `adb` / `idevice` / Sideloadly / Xcode
are on this machine, there's no remote host and no ssh. It pulls the build
artifact from the region's mirror (**global → GitHub Releases, CN → Aliyun OSS
Shanghai**; no R2), region auto-detected here or forced via
`CICY_INSTALL_REGION` / `cfg.region`.
...
- the phone is plugged into **this Mac** (the one running the skill),
...
```sh
cicy-mobile-install status            # region, latest version, tools, connected devices
