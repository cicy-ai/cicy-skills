...
- `github gh` auto-downloads the pinned GitHub CLI into `~/cicy-ai/runtime/gh/<version>/` when `gh` is not on `PATH` (`github gh-setup` to prefetch). Nothing outside that directory is modified.
