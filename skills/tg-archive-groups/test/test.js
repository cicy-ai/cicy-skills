...
if(a.includes('webcontents')) out={ok:true,data:[{webContentsId:113,url:'https://web.telegram.org/k/',title:'TG'},{webContentsId:9,url:'https://example.com/'}]};
  ...
  if(ex.includes('getDialogs')){ const inc=ex.includes('!true'); const bots=ex.includes('if (true) out.push'); const items=s.items.filter(x=>x.kind!=='service').filter(x=>x.kind==='bot'?bots:(inc||x.kind==='group')).map(x=>({...x,unread:0}));
...
test('archive --yes moves groups + channels; --groups-only spares channels; unarchive brings all back', () => {
  ...
  assert.equal(r.moved, 5); assert.deepEqual(st().items.map((x) => x.folder), [0, 0, 0, 0, 0, 0]);
});
