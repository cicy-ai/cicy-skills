...
function fakeAgent() {
  ...
  fs.writeFileSync(state, JSON.stringify({ items: [
    ...
    { peerId: '500', title: 'B1', kind: 'bot', folder: 0 }, { peerId: '777000', title: 'Telegram', kind: 'service', folder: 0 }], calls: [] }));
...
if(a.includes('webcontents')) out={ok:true,data:[{webContentsId:113,url:'https://web.telegram.org/k/',title:'TG'},{webContentsId:9,url:'https://example.com/'}]};
  ...
  if(ex.includes('getDialogs')){ const inc=ex.includes('!true'); const bots=ex.includes('if (true) out.push'); const items=s.items.filter(x=>(x.kind==='bot'||x.kind==='service')?bots:(inc||x.kind==='group')).map(x=>({...x,unread:0}));
...
test('archive --yes moves groups + channels; --groups-only spares channels; unarchive brings all back', () => {
  ...
  assert.equal(r.moved, 2); assert.equal(st().items.find((x) => x.kind === 'bot').folder, 1); assert.equal(st().items.find((x) => x.kind === 'service').folder, 1);
  r = JSON.parse(run(['unarchive', '--target', 'wc:113', '--yes', '--bots', '--delay', '0', '--json']));
  assert.equal(r.moved, 6); assert.deepEqual(st().items.map((x) => x.folder), [0, 0, 0, 0, 0, 0]);
});
