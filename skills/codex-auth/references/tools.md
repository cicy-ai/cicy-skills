# codex-auth — files & related commands

## Files it touches

| Path | Role |
|------|------|
| `~/.codex/auth.json` | the live Codex credential — read by `export`, written by `import` |
| `~/.codex/auth.json.bak-<timestamp>` | automatic backup made by `import` |
| `~/cicy-ai/assets/codex-auth-<timestamp>.b64` | default `export` destination, mode 0600 |

`CODEX_AUTH_PATH` overrides the credential path.
...
- The other half of this pair: `claude-auth`.
