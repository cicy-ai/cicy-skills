...
- installing the mihomo binary itself (`cicy-mihomo install` pulls the per-platform `cicy-mihomo-<plat>` subpackage from npm — same channel as cicy-code, into the `~/cicy-ai/runtime/mihomo/<ver>/` store; version-aware: present & current → skip, absent → install, behind → update; a GitHub release is the fallback. `--force` to reinstall)
